...
fn testnet_genesis(
    ...
    dev::GenesisConfig {
        ...
        pallet_democracy: Some(dev::DemocracyConfig::default()),
        pallet_treasury: Some(Default::default()),
        pallet_elections_phragmen: Some(dev::ElectionsConfig::default()),
        pallet_im_online: Some(dev::ImOnlineConfig { keys: vec![] }),
        pallet_authority_discovery: Some(dev::AuthorityDiscoveryConfig { keys: vec![] }),
        pallet_session: Some(dev::SessionConfig {
            keys: initial_authorities
...
pub fn malan_config() -> Result<DevChainSpec, String> {
    use hex_literal::hex;
    use sp_core::crypto::UncheckedInto;

    let wasm_binary =
        chainx::WASM_BINARY.ok_or_else(|| "Development wasm binary not available".to_string())?;
    ...
    Ok(DevChainSpec::from_genesis(
        ...
        constructor,
        bootnodes![
            "/dns/p2p.1.testnet.chainx.org/tcp/20222/p2p/12D3KooWCfqHSrd8UgLyNRRZwsVpkTt7SJ6yJHTRNCYhNXh2aYeV",
            "/dns/p2p.2.testnet.chainx.org/tcp/20222/p2p/12D3KooWHkjLFC9zQAaAMwxYvPs83uAXSM2fD4Ji1puZiz4FxAoG",
            "/dns/p2p.3.testnet.chainx.org/tcp/20223/p2p/12D3KooWRcmKCa1Uo54UNV6umzvVnWAx7TZNFibbZfP87zqPs1DP",
        ],
        Some(
